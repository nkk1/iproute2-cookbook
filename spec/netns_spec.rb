...
describe 'interface state' do
  ...
  context 'when in network namespace' do
    describe 'when normal interface' do
      let(:link) { IPRoute::Link.new('eth0', 'netns') }
      before do
      ...
      it 'sets interface state' do
        expect(Mixlib::ShellOut).to receive(:new).with('/sbin/ip netns exec netns /sbin/ip link set dev eth0 up')
                                                 .and_return(shellout)
        link.state = 'up'
      end
      ...
      it 'sets interface mtu' do
        expect(Mixlib::ShellOut).to receive(:new)
          .with('/sbin/ip netns exec netns /sbin/ip link set dev eth0 mtu 987654321').and_return(shellout)
        link.mtu = 987654321
      end
      ...
      it 'sets qlen' do
        expect(Mixlib::ShellOut).to receive(:new)
          .with('/sbin/ip netns exec netns /sbin/ip link set dev eth0 txqueuelen 123456789').and_return(shellout)
        link.qlen = 123456789
      end
      ...
      it 'sets mac' do
        expect(Mixlib::ShellOut).to receive(:new)
          .with('/sbin/ip netns exec netns /sbin/ip link set dev eth0 address a:b:c:d:e:f').and_return(shellout)
    ...
    describe 'link down' do
      let(:link) { IPRoute::Link.new('eth4', 'netns') }
      before(:each) do
    ...
    describe 'link alias' do
      ...
      it 'sets alias' do
        expect(Mixlib::ShellOut).to receive(:new)
          .with('/sbin/ip netns exec netns /sbin/ip link set dev eth0 alias "summa"').and_return(shellout)
        link.alias = 'summa'
      end
    end
  end
